...
import { createBuckets, deleteEntry, insertEntry, type Buckets } from './lib/hashmap';

function App() {
  const { progress, status, save, reset } = useProgress();
  const [buckets, setBuckets] = useState<Buckets>(createBuckets);
  ...
  const handleInsert = useCallback((key: string, value: string) => {
    let insertedIndex = 0;
    setBuckets((prev) => {
